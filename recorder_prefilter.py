...
#
#   This module fixes that problem by using a code injection process to
#   provide a local prefilter to determine if an entity should be added before
#   the Recorder filter.
#
#
...
#     recorder_prefilter.remove_filter(hass,
...
def _update_filter(hass: HomeAssistant, entities=None, remove=False):
    ...
    _LOGGER.info(
        f"{mode} Recorder Prefilter Entities ({cust_component})-{entities_cnt}"
    )
    ...
    _LOGGER.info(
        f"Recorder Prefilter Entities Updated, "
